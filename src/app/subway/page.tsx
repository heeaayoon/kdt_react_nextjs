...
export default function Subway() {
    ...
    const getFetchData = async() =>{
        ...
        let url = `${baseUrl}serviceKey=${apikey}&pageNo=1&numOfRows=24&resultType=json&controlnumber=${today}&areaIndex=${sRef.current?.value}`;
        //console.log(url) //확인용
  ...
  return (
    <div className='w-full flex flex-col'>
      <div className='flex gap-20'>
        <div className='mr-10 font-bold p-2'>측정소 선택</div>
            <TailSelect selRef = {sRef}
                        handelSel = {handleS}/> 
            {/* <TailSelect> 컴포넌트 수정함 */}
        </div>
        <div className='w-full flex flex-col mt-10 gap-4'>
          {tdata.map((item:TdataItem, idx:number) => <SubwayBox key={item.controlnumber}          
                                                                item = {item} 
                                                                idx={idx}/>)}
        </div>
    </div>
